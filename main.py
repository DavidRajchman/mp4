        ...
        try:
            ...
            for row in reader:
                if len(row) == 2:
                    ...
                    conn = sqlite3.connect('data.db')
                    c = conn.cursor()
                    # Insert the new temperature reading
                    c.execute("INSERT INTO temperatures VALUES (?, ?)", (timestamp, float(temperature)))
                    print("Inserted new temperature reading from serial")
                    conn.commit()
